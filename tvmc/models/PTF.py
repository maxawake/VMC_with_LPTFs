...
class FastMaskedTransformerEncoder(torch.nn.Module):
    """
...
class PTF(Sampler):
    ...
    def __init__(self, L, patch, Nh, dropout, num_layers, nhead, repeat_pre, device=DEVICE, **kwargs):
        ...
        self.tokenize = torch.nn.Sequential(torch.nn.Linear(self.p, Nh[0]), torch.nn.Tanh())
        ...
        self.lin = torch.nn.Sequential(
            torch.nn.Linear(Nh[1], (Nh[1] if repeat_pre else Nh[2])),
            torch.nn.ReLU(),
            torch.nn.Linear(Nh[0], 1 << self.p),
    ...
    def sample(self, B, L, h0=None):
        ...
        for idx in range(1, L + 1):
            ...
            onehot = torch.nn.functional.one_hot(indices, num_classes=1 << self.p)
    ...
    def off_diag_labels(self, sample, nloops=1):
        ...
        cache = (
            cache.repeat(1, 1, L // D, 1, 1)
            .transpose(2, 3)
            .reshape(cache.shape[0], L // self.p, B * L // D, cache.shape[-1])
        )
